...
function ProjectsRoute({ children }) {
  const { user } = useAuth();
  if (!user) return <Navigate to="/login" replace />;
  if (!['employee', 'projectmanager', 'department_head', 'admin', 'superadmin'].includes(user.role)) {
...
function AppRoutes() {
  return (
    <Routes>
      ...
      <Route path="/tasks"          element={<TasksRoute><Tasks view="tasks" /></TasksRoute>} />
      <Route path="/projects"       element={<ProjectsRoute><Tasks view="projects" /></ProjectsRoute>} />
