...
export default function AdminOverview() {
  ...
  useEffect(() => {
    ...
    const exists = admins.some((a) => String(a.id) === String(selectedAdminId));
    if (!exists) setSelectedAdminId(String(admins[0].id));
  }, [admins, selectedAdminId]);

  const selectedAdmin = admins.find((a) => String(a.id) === String(selectedAdminId));

  const visibleModules = useMemo(() => {
    return MODULES.filter((m) => (m.id === 'system' ? selectedAdmin?.role === 'superadmin' : true));
  }, [selectedAdmin?.role]);
