  ...
  return (
    <MainLayout title={t('sidebar.home', 'Главная')}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, color: 'var(--gray-900)', marginBottom: 16 }}>{tr('Актуальные новости')}</h2>
        {newsLoading && <div style={{ color: 'var(--gray-500)' }}>{t('common.loading', 'Загрузка...')}</div>}
        {!newsLoading && newsToShow.length === 0 && <div style={{ color: 'var(--gray-500)' }}>{tr('Новостей пока нет')}</div>}
        <div className="news-grid">
          {newsToShow.map((n) => (
            <div key={n.id} className="news-card" onClick={() => setSelectedNews(n)}>
              {n.img ? <img src={n.img} alt={n.title} className="news-card-img" onError={(e) => { e.currentTarget.style.display = 'none'; }} /> : null}
