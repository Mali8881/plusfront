...
export default function Schedule() {
  ...
  useEffect(() => {
    ...
    // Sync today's mark from backend attendance API into current page model.
    const syncTodayFromApi = async () => {
      try {
        const now = new Date();
        const y = now.getFullYear();
        const m = now.getMonth() + 1;
        const todayKey = `${y}-${String(m).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
        const res = await api.get('/v1/attendance/my/', { params: { year: y, month: m } });
        const marks = Array.isArray(res?.data) ? res.data : [];
        const todayMark = marks.find((x) => x?.date === todayKey);
        if (!todayMark) return;
        const checkMeta = {
          source: 'backend',
          mode: todayMark.status === 'remote' ? 'online' : 'office',
          syncedAt: new Date().toISOString(),
        };
        const rec = setCheckIn(user.id, todayMark.created_at || new Date().toISOString(), checkMeta);
        setDayMark(rec);
      } catch {
        // Keep local fallback behavior if backend mark fetch is unavailable.
      }
    };
  ...
  const handleCheckIn = () => {
    if (!user?.id) return;
    if (currentDayMode === 'dayoff') {
      setLateAlert('Сегодня выходной по графику. Отметка прихода недоступна.');
      return;
    }
    let checkMeta = {
      mode: currentDayMode,
    ...
    const workMode = currentDayMode === 'online' ? 'online' : 'office';
    api
      .post('/v1/attendance/check-in/', { work_mode: workMode })
      .then((res) => {
        const checkedAt = res?.data?.checked_at || new Date().toISOString();
        const rec = setCheckIn(user.id, checkedAt, {
          ...checkMeta,
          source: 'backend',
          inOffice: Boolean(res?.data?.in_office),
          ipValid: Boolean(res?.data?.ip_valid),
          plannedMode: res?.data?.planned_mode || workMode,
        });
        setDayMark(rec);
        setLateAlert('');
      ...
      .catch((err) => {
        setLateAlert(getApiErrorText(err, 'Не удалось выполнить отметку прихода.'));
      });
  };
  ...
  const handleCheckOut = () => {
    ...
    const status = currentDayMode === 'online' ? 'remote' : 'present';
    api
      .patch('/v1/attendance/mark/', {
        date: todayDateKey,
        status,
        comment: `Check-out at ${checkedOutAt}`,
      })
      .then(() => {
        const rec = setCheckOut(user.id, checkedOutAt, { ...checkMeta, source: 'backend' });
        setDayMark(rec);
        setLateAlert('');
      })
      .catch((err) => {
        setLateAlert(getApiErrorText(err, 'Не удалось выполнить отметку ухода.'));
      });
  };
